	...
	for (var i = 0; i < this.data.length; i++) {
		...
		for (var j = 0; j < childrens.length; j++) {
			...
			tableBodyTr.click(function(event) {
				...
				if (_this.onrowclick) {
					// 找到当前点击行元素之前对应的所有父级行
					var trParents = $(this).prevAll().find(".group");
					// 如果详细数据有多列, 父级行除了标题就会有多个&nbsp; ,所以这里将真正的标题筛选出来
					var realParens = [];
					for (var i = 0; i < trParents.length; i++){
						var trItem = trParents[i];
						if ($(trItem).html() != "" && $(trItem).html() != "&nbsp;"){
							realParens.push(trItem);
						}
					}
					// 最后一个就是当前点击行对应的真正父级行
					// 方式1
					var parentTrIndex = realParens.length - 1;
					// 方式2
//					var parentTrIndex = -1;
//					var trDom = $(realParens).last();
//					parentTrIndex = $(realParens).index($(trDom));
					
					// 获取到正确的父级行后要获取点击行在同级数据中的Index, 以行内的group属性作为索引
					var thisGroupSign = $(this).attr("group");
					// 根据group属性找到所有同级行数据
					var sameTrs = $("tr[group=" + thisGroupSign + "]");
					// 在同级行数据中找到当前点击行的Index
					var itemIndex = $(sameTrs).index($(this));
					// 最后返回的数据是从正确的父级行数据中取得代表详细子数据行的items, 再从items中用当前点击行的index取得我们真正想要的数据
					var returnItem = _this.data[parentTrIndex].items[itemIndex];
					// 返回真正的当前行数据
					_this.trigger(_this.onrowclick, returnItem);
				}
