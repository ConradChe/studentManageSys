...
hait.grid.Grid.prototype.draw = function() {
	...
	// 初始化表头
	var tableHead = $('<div class="hait-table-title"></div>');
	tableHead.append('<table cellspacing="0" cellpadding="0" border="0" id="targetFirstGrid" class="hait-table">'
			+ '<thead></thead></table>');
	...
	if (this.header == null || this.header.length == 0) {
		...
		tableHead.find("thead").append(tableHeadTr);
	} else {
		// 将预设的表头设置进去
		tableHead.find("thead").append(this.header);

		// 初始化预设的表头内部每列的宽度
		tableHead.find("tr").each(function() {
			var index = 0;
		...
		if (this.multiple) {
			var tableHeadTrTh = $("<th class='checkbox'></th>");
			tableHeadTrTh.attr("width", this._checkboxWidth + "%");
			if (tableHead.find("tr").size() > 1) {
				tableHeadTrTh.attr("rowspan", tableHead.find("tr").size());
			}
			tableHead.find("tr:first").prepend(tableHeadTrTh);
		}
	}

	// 如果需要表头点击排序，那么对表头增加事件
	if (this.titleSort) {
		tableHead.find("th").click(function() {
			...
			// 清除当前所有排序样式
			$(this).parent().parent().find(".sort-asc").removeClass("sort-asc");
			$(this).parent().parent().find(".sort-desc").removeClass("sort-desc");
			// 设置当前样式
...
hait.grid.Grid.prototype._fillData = function() {
	...
	for (var i = 0; i < this.data.length; i++) {
		...
		for (var j = 0; j < this.columns.length; j++) {
			...
			// 如果是第一行，设置宽度
			if (i == 0) {
				tableBodyTrTd.attr("width", this.columns[j].width + "%");
			}
	...
	// 如果存在滚动，那么调整界面显示
	if (this._isScroll) {
		this.obj.find(".hait-table-title > table").css("border-right", "1px solid " + showTypeColor);
		this.obj.find(".hait-table-title").css("padding-right", scrollPaddingRight);
	} else {
		this.obj.find(".hait-table-title > table").css("border-right", "0px");
		this.obj.find(".hait-table-title").css("padding-right", "0px");
	}
	...
	this.obj.find(".hait-table-data tr:last > td").css("border-bottom", this._isLargeDataArea ? "1px solid " + showTypeColor : "0px");
...
hait.grid.Grid.prototype._getStatisticsDate = function() {
	...
	if (tableStatistics.size() == 0) {
		tableStatistics = $('<div class="hait-table-statistics"></div>');
		this.obj.append(tableStatistics);
	}
...
hait.grid.Grid.prototype._fillStatisticsHtml = function() {
	...
	tableBodyTr.append("<td width='" + this.columns[0].width + "%'>" + (_this.statFuncId ? "远程统计" : "本地统计") + "</td>");

	// 如果没有数据，那么这里需要设置宽度
	if (this.data.length == 0) {
		tableBodyTr.find("td:last").attr("width", this.columns[0].width + "%");
	}
	...
	if (!this.pagination) {
		tableBodyTr.find("td").css("border-bottom", "0px");
	}

	// 将这行添加到底部窗口
	tableStatistics.find("tbody").append(tableBodyTr);
	...
	// 如果存在滚动，那么调整界面显示
	if (this._isScroll) {
		this.obj.find(".hait-table-statistics > table").css("border-right", "1px solid " + showTypeColor);
		this.obj.find(".hait-table-statistics").css("padding-right", scrollPaddingRight);
	} else {
		this.obj.find(".hait-table-statistics > table").css("border-right", "0px");
		this.obj.find(".hait-table-statistics").css("padding-right", "0px");
	}
};
...
hait.grid.Grid.prototype._fillPagination = function() {
	...
	// 拼装分页容器
	var pagination = $('<div class="pagination-bar fr"></div>');

	// 添加分页描述
	pagination.append('<div class="pagination-info">共有' + this._total + '条， 每页显示：' + this.limit + '条</div>');
	...
	paginationNumbers.append('<li name="prevBtn">&lt;</li>');
	...
	paginationNumbers.append('<li name="nextBtn">&gt;</li>');
	pagination.append(paginationNumbers);

	// 添加分页查询框
	pagination.append('<input type="text" class="pagination-input"/>');
	pagination.append('<div class="pagination-search">查</div>');
	...
	var jumpBtn = pagination.find(".pagination-search");
	jumpBtn.click(function() {
		var jumpNumber = pagination.find(".pagination-input").val();
...
hait.grid.Grid.prototype._initColumnWidth = function() {
	...
	for (var i = 0; i < this.columns.length; i++) {
		...
		if (curWidth.indexOf("%") >= 0) {
			...
			curWidth = parseInt(curWidth);
			curWidth = Math.ceil(curWidth / this.obj.width() * 100);
		} else {
			// 没有按规则设置，都清空
			curWidth = null;
		}
	...
	// 计算复选框宽度
	if (this.obj.width() > 0) {
		var curCheckboxWidth = Math.ceil(35 / this.obj.width() * 100);
	...
	if (defaultWidth < 0) {
		...
		defaultWidth = Math.floor(defaultWidth); // 获取最接近的整数
		for (var i = 0; i < this.columns.length; i++) {
